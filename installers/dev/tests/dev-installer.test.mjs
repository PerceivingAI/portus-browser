...
test("creates browser-specific macOS and Linux native messaging plans", () => {
  ...
  assert.equal(linuxPlan.nativeHosts[0].manifestPath, "/home/portus-test/.config/microsoft-edge/NativeMessagingHosts/com.portus.browser.json");
  ...
  const macPlan = createDevInstallPlan({
    ...
    extensionId,
    env: {
      HOME: "/Users/portus-test"
    }
  });

  assert.equal(macPlan.nativeHosts[0].manifestPath, "/Users/portus-test/Library/Application Support/BraveSoftware/Brave-Browser/NativeMessagingHosts/com.portus.browser.json");
});
